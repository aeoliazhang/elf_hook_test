...
uint32_t find_got_entry_address(const char *module_path,
	...
	if (module_base == 0) {
		LOGE("[-] it seems that process %d does not dependent on %s",
				getpid(), module_path);
		return 0;
	}

	LOGE("[+] base address of %s: 0x%x", module_path, module_base);
	...
	if (read(fd, elf_header, sizeof(Elf32_Ehdr)) != sizeof(Elf32_Ehdr)) {
		LOGE("[-] read %s error! in %s at line %d",
	...
	LOGE("[+] start of section headers: 0x%x", sh_base);
	LOGE("[+] section header string table index: %d", ndx);
	LOGE("[+] section header string table offset: 0x%x", shstr_base);
	...
	LOGE("[+] section header string table offset: 0x%x", shstr_shdr->sh_offset);
	...
	Elf32_Shdr *exidx_shdr = (Elf32_Shdr *) malloc(sizeof(Elf32_Shdr));
	...
	for (; i < elf_header->e_shnum; i++) {
		...
		else if (strcmp(s, ".ARM.exidx") == 0) {
			memcpy(exidx_shdr, shdr, sizeof(Elf32_Shdr));
			LOGE("[+] .ARM.exidx  sh_offset = %d, sh_entsize = %d",
					exidx_shdr->sh_offset, exidx_shdr->sh_entsize);
		}

		if (read(fd, shdr, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr)) {
			LOGE("[-] read %s error! i = %d, in %s at line %d",
					module_path, i, __FILE__, __LINE__);
			return 0;
		}
	}

	LOGE("[+] offset of .rel.plt section: 0x%x", relplt_shdr->sh_offset);
	LOGE("[+] offset of .dynsym section: 0x%x", dynsym_shdr->sh_offset);
	LOGE("[+] offset of .dynstr section: 0x%x", dynstr_shdr->sh_offset);
	...
	for (i = 0; i < relplt_shdr->sh_size / sizeof(Elf32_Rel); i++) {
		...
		LOGE("[-] Func: %s", dynstr + dynsymtab[ndx].st_name);
		if (strcmp(dynstr + dynsymtab[ndx].st_name, symbol_name) == 0) {
			LOGE("[+] got entry offset of %s: 0x%x",
...
uint32_t do_hook(const char *module_path, uint32_t hook_func,
	...
	LOGE("[+] found symbol %s addr=0x%x", symbol_name, entry_addr);

	uint32_t original_addr = 0;
	// save original GOT entry content
	memcpy(&original_addr, (uint32_t *) entry_addr, sizeof(uint32_t));

	LOGE("[+] hook_fun addr: 0x%x", hook_func);
	LOGE("[+] got entry addr: 0x%x", entry_addr);
	LOGE("[+] original addr: 0x%x", original_addr);

	uint32_t page_size = getpagesize();
	uint32_t entry_page_start = PAGE_START(entry_addr, page_size);
	LOGE("[+] page size: 0x%x", page_size);
	LOGE("[+] entry page start: 0x%x", entry_page_start);
...
int main(int argc, char **argv) {
	...
	ret = do_hook(argv[0], (uint32_t) mygetpid, "getpid");
	if (ret == 0) {
		LOGE("ERROR: hook failed!");
	}

	LOGE("new pid = %d", getpid());
	LOGE("org pid = %d", syscall(__NR_gettid));
	return 0;
}
